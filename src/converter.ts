...
export function getBiomeToUse(staticMesh: StaticMesh, overrideMaterials: OverrideMaterials) {
	const globalState: GlobalState = GlobalStore.getState();
	const finalTypes: string[] = [];
	if (typeof staticMesh !== "undefined" && typeof overrideMaterials !== "undefined") {
	...
	finalTypes.forEach((element) => {
		...
		if (currentData) {
			if (currentData.ObjectPath && currentData.ObjectName) {
				portedMaterialObjects.push(currentData);
			}
		} else {
	...
	for (let index = 0; index < dataObjs.length; index++) {
		...
		if (!lengthGreater2) {
			...
			if (
				...
				typeof biomeKey !== "undefined"
			) {
				const ogMat = overrideMaterials;
