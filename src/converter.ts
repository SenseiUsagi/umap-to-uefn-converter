...
// Rewrite this for recursive calling
    ...
    for (let index = 0; index < dataObjs.length; index++) {
        ...
        if (!lengthGreater2) {
            ...
            if (
                ...
                staticMesh.MeshData !== null &&
                useOverrideBiome(globalState)
            ) {
                overrideMaterials = getBiomeToUse(
                    staticMesh,
                    overrideMaterials
                );
            }
    ...
    if (lengthGreater2) {
        ...
        if (location !== undefined) {
            ...
            const biomeKey = (
                ...
                (key) => globalState.currentSettings.overrideBiome[key] === true
            );
...
export function convertToUEFN(
    ...
    folderName: string,
    isLevel: boolean,
    locationOffset: RelativeLocation | null | undefined,
    rotation: RelativeRotation | null | undefined
): convertedLevel {
