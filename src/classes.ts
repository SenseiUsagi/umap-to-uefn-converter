    ...
    convertToUEFN(): string {
        ...
        for (let index = 0; index < this.Materials.length; index++) {
            ...
            if (element === null) {
                ...
                if (globalState.currentSettings.usePortedModels) {
                    const portedMaterial: String | undefined =
                        portedMaterialsPaths[tempMeshName];
                    ...
                    if (typeof portedMaterial !== "undefined") {
                        material = `/${
                            globalState.currentSettings
                                .portedModelsProjectName + portedMaterial
                        }`;
                    }
        ...
        if (this.LODData) {
            this.LODData.forEach((element, index) => {
                if (element.OverrideVertexColors !== undefined) {
                    ...
                    if (
                        typeof element.OverrideVertexColors.Data !== "undefined"
                    ) {
                        if (
                            ...
                            element.OverrideVertexColors.Data.forEach(
                                (color, colorIndex) => {
                                    colorData += `${color}${
                                        colorIndex !==
                                        element.OverrideVertexColors!.Data
                                            .length -
                                            1
                                            ? ","
                                            : ""
    ...
    convertToUEFN(): string {
        ...
        const textureData: string =
            this.data.textureData !== null
                ? this.data.textureData!.convertToUEFN()
                : "";
